...
const API_Key = 'api_key=ca4c7c42776a54a44956cc5ee120ba50';
const BaseURL = 'https://api.themoviedb.org/3';
const q = '/discover/movie?';

const url = BaseURL + q + API_Key

const imageURL = 'https://image.tmdb.org/t/p'
function getMovies(urlink){
    fetch(urlink)
    ...
    .then(data =>{
        ...
        for (let i = 1; i < languages.length; i++){
           $('#languages').append(`<option value="${languages[i]["iso_639_1"]}">${languages[i]["english_name"]}</option>`)
        }

        for (let i = 0; i < region.length; i++){
         $('#region').append(`<option value="${region[i]["iso_3166_1"]}">${region[i]["english_name"]}</option>`)
      }
        ...
        $.each(data.results, function (index, movie){
            ...
            $('.content').append(`<div class="movie movie${index}">
        <h6 class="title">(${index + 1}) ${movie.title}</h6>
        <img class='poster' src="${poster_path? imageURL + '/w185' + poster_path: "https://assets.prestashop2.com/sites/default/files/styles/blog_750x320/public/blog/2019/10/banner_error_404.jpg?itok=eAS4swln" }" alt="Nothing yet">
        <h6>Rating: <span class="movie-rating">${movie.vote_average}</span></h6>
...
function filter(){
    ...
    var date_start = $('#date-start').val()? "primary_release_date.gte=" + $('#date-start').val() + "&": ''
    var date_end = $('#date-end').val()? "primary_release_date.lte=" + $('#date-end').val() + "&": ''
    var vote_average_start = $('#vote-average-start').val()? "vote_average.gte=" + $('#vote-average-start').val() + "&": ''
    var vote_average_end = $('#vote-average-end').val()? "vote_average.lte=" + $('#vote-average-end').val() + "&": ''
    var with_genres = encodeURI(selectedGenre.join(','))? "with_genres=" + encodeURI(selectedGenre.join(',')) + "&": ''
    var page = "page=" + $('#pages').val() + '&'
    var sorting = $('#inputGroupSelect01').val()? "sort_by=" +$('#inputGroupSelect01').val() + "&": ''
    var languages = $('#languages').val() != 'xx' || $('#languages').val() != ''? "language=" + $('#languages').val() + '&': ''
    var region = $('#region').val() != ''? "region=" + $('#region').val() + '&': ''

    let url_filter = BaseURL + q + date_start + date_end + vote_average_start + vote_average_end + with_genres+ page+ sorting + languages + region + API_Key
    console.log('This is the fetching url: ' + url_filter)

    getMovies(url_filter)
...
// The following code for genres is referred to the demonstration and github code
// The owner of the code: Asish George Tech (Youtube) / asishgeorge (Github)
// Title of his project: 50Projects50Days
